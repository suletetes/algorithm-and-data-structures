...
void case3(struct Node *p, struct Node *q, struct Node *w, int lChild, int rChild, int parent, int top, int x) {
    ...
    newParent->right = r;
    if (q->isRoot)
        newRoot = newNode(newRoot, top,
                          true); // We know the parent node's value because if we are on the left child, whichever key we pick will be smaller than any of the parent's keys
    ...
    if (q->left == p) { // If we are on the left child
        ...
        return;
               ...
               q->middle == p) { // If we are on the middle child and there is a parent we attach our new root to
...
void iInsert(struct Node *p, int x) {
    ...
    if (!p->left && !p->right && !p->middle) {
        ...
        while (p != NULL) {
            ...
            if (q != NULL && q->keys[0] != -1 && q->keys[1] !=
                                                 -1) { // If parent xx we may have case 3 depending on whether p is xx or not and what child p is
